...
<div class="flex flex-col gap-6 font-sans">
  ...
  <div class="flex flex-wrap items-center gap-2 border-b border-white/10 pb-3">
    {#each roles as r (r.position)}
      <button
        class="rounded-md px-3.5 py-1.5 text-xs font-semibold uppercase tracking-wider transition-all {selectedPos === r.position ? 'bg-zinc-200 text-zinc-950 shadow' : 'bg-zinc-900/80 text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200'}"
  ...
  {#if currentRole}
    {@const roleName = getRoleName(currentRole.position)}
    ...
    <div class="flex flex-col gap-7">
      ...
      <section class="flex flex-col gap-3">
        <h3 class="font-sans text-[13px] font-bold uppercase tracking-[0.14em] text-[#8e9aa8]">
          TOP {roleName} - ORDERED BY D2PT ELO
        </h3>
        ...
        <div class="flex flex-wrap items-start gap-x-3 gap-y-4">
          {#each topHeroes as h (h.hero_id)}
            <div class="group flex w-[62px] flex-col items-center select-none" title="{h.name} • {pct2(h.winrate)} WR • {pct2(h.pickrate)} Pick">
              ...
              <div
                class="relative h-[96px] w-[62px] overflow-hidden border border-[#2b3542] bg-[#10141a] shadow-[0_3px_10px_rgba(0,0,0,0.8)] transition-all group-hover:border-[#60a5fa] group-hover:shadow-[0_0_10px_rgba(96,165,250,0.3)]"
              >
                <img
                  src={d2ptVert(h.slug)}
                  alt={h.name}
      ...
      {#if otherHeroes.length > 0}
        <section class="flex flex-col gap-3">
          <h3 class="font-sans text-[13px] font-bold uppercase tracking-[0.14em] text-[#8e9aa8]">
            OTHER {roleName} - ORDERED BY D2PT RATING
          </h3>
