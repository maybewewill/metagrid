...
import { getPortraitDir, getSnapshot, saveSettings } from "$lib/ipc";
...
describe("ipc", () => {
  ...
  it("getPortraitDir calls invoke('get_portrait_dir')", async () => {
    invoke.mockResolvedValue("C:/data/portraits");
    await getPortraitDir();
    expect(invoke).toHaveBeenCalledWith("get_portrait_dir");
  });
});
